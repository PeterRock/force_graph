...
export default class D3Force extends Component {
  ...
  drawData() {
    ...
    const node_x = []
    const node_y = [] //save the absolute coordinate of nodes
    ...
    const drawNode = node => {
      let { gfx } = node
      gfx.lineStyle(1, PIXI.utils.string2hex(node.borderColor))
        .beginFill(PIXI.utils.string2hex(node.background))
    ...
    function ticked() {
      ...
      data.links.forEach((link, k) => {
        ...
        links.lineStyle(1, PIXI.utils.string2hex(LINE_COLOR), 0.5)
  ...
  render() {
    ...
    return (
      <div style={{ touchAction: 'none', width, height }}>
        <div
          ref={canvas => {
            this.canvas = canvas
          }}
